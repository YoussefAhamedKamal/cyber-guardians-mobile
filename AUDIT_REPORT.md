...
**Date:** 2026-06-23  
**Version:** v1.2.0 (Agent) / v10.0.0 (Game)  
...
| Area | Status | Notes |
|------|--------|-------|
| Shell injection (OpenCode) | ⚠️ BUG-004 | Prompts not fully sanitized |
...
1. **BUG-001/002**: Fix HuggingFace provider (completely broken)
2. **BUG-004**: Fix shell injection (security risk)
3. **BUG-003**: Fix AI fallback race condition (concurrent request bugs)
4. **BUG-005**: Connect grep include filter (cosmetic → functional)
5. **BUG-006**: Fix OpenCode provider buttons (cosmetic → functional)
6. Rest can be addressed incrementally
